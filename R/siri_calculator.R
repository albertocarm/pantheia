...
#' @importFrom stats model.frame model.matrix delete.response median
...
siri_calculator <- function() {
  ...
  if (!exists("mod_pfs") || !exists("mod_os") || !exists("mod_resp")) {
    stop("Internal models (mod_pfs, mod_os, mod_resp) not found. Ensure sysdata.rda is loaded correctly.")
  }
  
  # =============================================================================
  # 2. PREDICTION ENGINES (Internal Helpers)
  # =============================================================================
  
  predecir_final <- function(modelo, newdata) {
    # A. PREPARATION
    knots_logsiri <- modelo$knots 
    trms <- stats::delete.response(modelo$terms)
    environment(trms) <- environment()
    
    # B. DESIGN MATRIX
    mf <- stats::model.frame(trms, newdata, xlev = modelo$xlevels, na.action = stats::na.pass)
  ...
  predecir_respuesta <- function(modelo, newdata) {
    ...
    if (regimen == "Gem-Abraxane") {
      ...
      # Note: regimen_catOther:logsiri is NA in the model, so we don't add it
    } else {
  ...
  # FILTRO: Eliminamos "Other" de las opciones de régimen
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        div(
          tags$label(
            "SIRI (absolute value):",
            tags$span(class = "info-icon", title = "Systemic Inflammation Response Index. Formula: SIRI = (Neutrophils × Monocytes) / Lymphocytes. Calculated from absolute blood cell counts.", "?")
          ),
  ...
  server <- function(input, output) {
    ...
    observeEvent(input$btn_calc, {
      ...
      # Patient dataframe (without edad)
      df <- data.frame(
